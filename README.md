...
- **RESTful API接口 (`api.php`)**: 🆕 标准API数据接口，支持AI智能分析和第三方集成。
- **API密钥管理 (`api_keys.php`)**: 🆕 安全的API密钥管理系统，支持密钥生成、权限控制和使用日志。
...
### v2.8.1: API接口集成 (正式版) 🆕
- [x] **RESTful API**: 标准HTTP接口，支持6个数据端点
- [x] **API密钥管理**: 安全的密钥生成、权限控制和使用日志
- [x] **Python客户端**: 开箱即用的API客户端脚本
- [x] **AI集成**: 支持AI助手直接访问数据进行分析
- [x] **数据汇总**: 智能统计和预警报告生成
...
4. 执行 `api_keys.sql` 创建API相关数据表。
5. 访问 `dashboard.php` 体验新功能。
6. 访问 `api_keys.php` 创建API密钥。

**注意**: v2.8.1 新增了2个API数据表（`api_keys`, `api_logs`），升级时需手动执行SQL。
